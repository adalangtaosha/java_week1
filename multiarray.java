...
//        声明每门课的名字
        ...
        boolean bool = true;//增加了bool，控制当输入yearcount<=0时，需要重新输入大于0的数字
        while(bool){
            System.out.println("请问要保存几年的成绩？");
            int yearcount = IN.nextInt();
            if (yearcount<=0){
                System.out.println("输入年数异常，请重新输入大于0的数");
            } else {
                int[][] scores = new int[yearcount][names.length];//成绩由原来的double直接定义为int
                for (int i = 0; i < yearcount; i++) {
                    for (int j = 0; j < names.length; j++) {
                        scores[i][j] = (int) (80 + Math.random() * 20);
